...
class CollectionsCache {
  final Map<int, List<GetCollectionDataEntity>> _cache = {};
  ...
  List<GetCollectionDataEntity>? getPage(int page) => _cache[page];

  void cachePage(int page, List<GetCollectionDataEntity> data) {
  ...
  void evictOldestPage() {
    if (_cache.isNotEmpty) {
      int oldestPage = _cache.keys.first;
  ...
  void logCacheUsage() {
    if (kDebugMode) {
      print('Cache size: ${_cache.length} pages, Total Collections Cached Items: ${totalCachedItems.value}');
    }
  }
}
