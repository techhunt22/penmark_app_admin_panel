...
class OrdersCache {
  final Map<int, List<OrdersDataEntity>> _cache = {};
  ...
  List<OrdersDataEntity>? getPage(int page) => _cache[page];

  void cachePage(int page, List<OrdersDataEntity> data) {
  ...
  void logCacheUsage() {
    if (kDebugMode) {
      print('Cache size: ${_cache.length} pages, Total Orders Cached Items: ${totalCachedItems.value}');
    }
  }
}
